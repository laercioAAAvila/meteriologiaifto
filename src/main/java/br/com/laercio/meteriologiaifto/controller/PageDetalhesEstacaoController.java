...
@Controller
public class PageDetalhesEstacaoController {
	...
	public String inicio(Model model, @PathVariable("id") int estacaoId) {
		return findPaginatedById(1, "temperatura", "asc", model, estacaoId);
	}

	@RequestMapping(method = RequestMethod.GET, value = "/estacao/{id}/{pageNo}")
	public String findPaginatedById(@PathVariable(value = "pageNo") int pageNo,
			@RequestParam("sortField") String sortField, @RequestParam("sortDir") String sortDir, Model model,
			@PathVariable("id") int estacaoId) {
		int pageSize = 5;

		Page<DadosMeteriologicos> page = dadosMeteriologicosService.findPaginatedById(pageNo, pageSize, sortField, sortDir, estacaoId);
